...
epsilon(1:420,1:zdim)=10.0*epsilon0;
...
for n=1:1:time_tot
    ...
    if gaussian==1
        ...
        elseif sourceType==2   %guassian, opposing pair
            ...
            Ez(xsource2i,zi) =   exp(-(n*deltat -600e-15- (z-z0)*sin(theta)/c*ng ).^2/sigmat^2).* ...
                exp(-(z-z0).^2*cos(theta)^2/sigmaz^2) ;
        ...
        elseif sourceType==3  %gaussian derivative, opposing pair
            ...
            Ez(xsource2i,zi) =   ttt.*exp(-(n*deltat -600e-15- (z-z0)*sin(theta)/c*ng ).^2/sigmat^2).* ...
                exp(-(z-z0).^2*cos(theta)^2/sigmaz^2) ;
...
tall = deltat*[1:time_tot];

%%%%% Ez
subplot(231)
imagesc(tall*1e12,z*1e3,Ezt1)
set(gca,'YDir','normal')
xlabel 'time [ps]'
ylabel 'z [mm]'
title 'E_z(z,x=2.1mm,t)'
grid on
 hold on; plotspj(2,tall*1e12,3e8*tall*1e3+1)

subplot(234)
imagesc(tall*1e12,z*1e3,Ezt2)
set(gca,'YDir','normal')
xlabel 'time [ps]'
ylabel 'z [mm]'
title 'E_z(z,x=2.5mm,t)'
grid on
 hold on; plotspj(2,tall*1e12,3e8*tall*1e3+1)

%%%%% Ex
subplot(232)
imagesc(tall*1e12,z*1e3,Ext1)
set(gca,'YDir','normal')
xlabel 'time [ps]'
ylabel 'z [mm]'
title 'E_x(z,x=2.1mm,t)'
grid on
 hold on; plotspj(2,tall*1e12,3e8*tall*1e3+1)

subplot(235)
imagesc(tall*1e12,z*1e3,Ext2)
set(gca,'YDir','normal')
xlabel 'time [ps]'
ylabel 'z [mm]'
title 'E_x(z,x=2.5mm,t)'
grid on
 hold on; plotspj(2,tall*1e12,3e8*tall*1e3+1)
...
%%%%% Hy
subplot(233)
imagesc(tall*1e12,z*1e3,Hyt1)
set(gca,'YDir','normal')
xlabel 'time [ps]'
ylabel 'z [mm]'
title 'H_y(z,x=2.1mm,t)'
grid on
 hold on; plotspj(2,tall*1e12,3e8*tall*1e3+1)


subplot(236)
imagesc(tall*1e12,z*1e3,Hyt2)
set(gca,'YDir','normal')
xlabel 'time [ps]'
ylabel 'z [mm]'
title 'H_y(z,x=2.5mm,t)'
grid on
 hold on; plotspj(2,tall*1e12,3e8*tall*1e3+1)
